 ...
 * 
 * The official SendX Wordpress plugin.
 * 
 * @package SendX
 * @global object $WP_SendX
 * @author Team SendX <hello@sendx.io>
 * @version 1.0.1
 */
/*
Plugin Name: Email Marketing by SendX
Plugin URI: https://sendx.io/?utm_source=wp-plugin&utm_medium=link&utm_campaign=plugins
Description: [SendX](https://sendx.io) is a lead-generation and marketing automation platform to grow your web business. In simple words it is marketing for non-marketers. Installing this plugin will provide you various widgets like hello bars, popups, sliders, modals etc. which you can use to convert your visitors to blog subscribers. 
Version: 1.0.1
Author: SendX
Author URI: http://sendx.io
License: GPLv2
...
class WP_SendX {
	var $namespace = "sendx";
	var $friendly_name = "SendX";
	var $version = "1.0.1";
	var $options_name = "sendx_options";
	var $options;
	var $settings_path;
	...
	public function __construct() {
		...
		$libs = glob( WP_SENDX_DIRNAME . '/lib/*.php' );
		foreach( $libs as $lib ) {
			include_once( $lib );
		}
	...
	/**
	 * Look up the team ID from the options array
	 *
	 * @return mixed Returns the team ID or NULL if it is not set or empty
	 */
	...
	/**
	 * Output the SendX tracking script
	 *
	 * Displays nothing if tracking script is disabled or the team ID is not set.
	 */
	public function tracking_code() {
		...
		if ( ! $this->get_option( 'is_disabled' ) ) {
			if ( $this->get_option( 'account_id' ) ) { 
				...
				include( WP_SENDX_DIRNAME . "/views/tracking-code.php" );
			} else {
				echo '<!-- SendX: Set your team ID to begin tracking -->';
			}
		}
	}
	...
	public function admin_menu() {
		$page_hook = add_options_page( 'SendX Settings', $this->friendly_name, 'manage_options', $this->namespace, array( &$this, 'admin_options_page' ) );
	...
	public function admin_options_page() {
		...
		include( WP_SENDX_DIRNAME . "/views/options.php" );
	}
	...
	public function admin_register_settings() {
		...
		add_settings_section( 'sendx_code_settings', 'Tracking Code', array( &$this, 'admin_section_code_settings' ), $this->namespace );
		add_settings_field( 'sendx_account_id', 'Team Id', array( &$this, 'admin_option_account_id' ), $this->namespace, 'sendx_code_settings' );
	}
	...
	/** 
	 * Output the input for the team ID option
	 */
	public function admin_option_account_id() {
		echo "<input type='text' name='sendx_options[account_id]' size='20' value='{$this->get_option( 'account_id' )}'>";
	}
	...
	public function admin_section_code_settings() {
		echo '<p>You can find your Team ID under <a href="http://app.sendx.io/setting" target="_blank">Settings &rarr; Site Setup</a> in your SendX account.</p>';
	}
	...
	function admin_enqueue_scripts() {
		wp_enqueue_style( "{$this->namespace}_admin_css", WP_SENDX_URLPATH . "/css/admin.css" );
	}
	...
	static function instance() {
		...
		if( ! isset( $WP_SendX ) ) $WP_SendX = new WP_SendX();
	}
}
